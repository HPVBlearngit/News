...
public interface TinTucDAO extends CrudRepository<TinTucModel, Integer>{
    ...
    @Query(value = "select top (1) * from tin_tuc where tin_tuc.the_loai_id = ?1 order by tin_tuc.id desc", nativeQuery = true)
    TinTucModel getTop1TinTucTheoTheLoai(Integer theLoai_id);
    
    @Query(value = "select * from tin_tuc where tin_tuc.the_loai_id = ?1 order by tin_tuc.id desc offset 1 rows fetch next 3 rows only", nativeQuery = true)
